...
public class SolrConnection extends DataConnection<SolrClient> {
	...
	interface SolrHttpContext {
		...
		static CloseableHttpClient createDefaultHttpclientBySolr() {
			...
			HttpClientUtil.addRequestInterceptor(SolrHttpContext.SOLR_HTTP_ICPT);
	...
	public static class SolrMeta implements Serializable {
		...
		/**
		 * @param uri
		 * @return Tuple3: <baseURL, defaultColl[maybe null], allColls>, or null for invalid uri
		 * @throws IOException
		...
		private static SolrMeta parse0(URISpec base, String file) throws IOException {
			...
			try (SolrConnection solr = new SolrConnection(base, false);) {
				...
				if (!(val instanceof ArrayList)) throw new RuntimeException("resp's coll type is incorrect!");
	...
	public void construct(String table, String aliasName, TableDesc tableDesc, List<FieldDesc> fields) {
		...
		try (CloudSolrClient client = new CloudSolrClient.Builder().withZkHost(solrUrl).sendUpdatesToAllReplicasInShard()
			...
			CollectionAdminRequest.Create request = CollectionAdminRequest.createCollection(tableName, DEFAULT_CORE_NAME, numShards,
					numReplicas);
