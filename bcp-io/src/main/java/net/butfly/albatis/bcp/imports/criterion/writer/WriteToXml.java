...
public class WriteToXml {
    ...
    public void addFields(String[][] fields) {
        if (fields == null || fields[0] == null || fields[0].length != 2) {
            return;
        }
        ...
        for (int i = 0; i < fields.length; i++) {
            Element item = element.addElement("ITEM");
            item.addAttribute("key", fields[i][0] == null ? "" : fields[i][0]);
            item.addAttribute("chn", fields[i][1] == null ? "" : fields[i][1]);
            item.addAttribute("eng", fields[i][0] == null ? "" : fields[i][0]);
        }
    }
